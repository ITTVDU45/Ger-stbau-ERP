...
interface ZahlungsbedingungenDialogProps {
  aktuellerText: string
  onAuswahl: (text: string) => void
  typ: 'auswahl' | 'speichern'
}

export default function ZahlungsbedingungenDialog({ aktuellerText, onAuswahl, typ }: ZahlungsbedingungenDialogProps) {
  const [open, setOpen] = useState(false)
  const [vorlagen, setVorlagen] = useState<ZahlungsbedingungenVorlage[]>([])
  ...
  const loadVorlagen = async () => {
    try {
      setLoading(true)
      const response = await fetch('/api/zahlungsbedingungen-vorlagen')
      if (response.ok) {
  ...
  const handleAuswahl = (vorlage: ZahlungsbedingungenVorlage) => {
  ...
  const handleSpeichern = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setName('')
        setKategorie('')
        toast.success('Zahlungsbedingungen erfolgreich gespeichert', {
    ...
    return (
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto bg-white">
          <DialogHeader>
            <DialogTitle className="text-gray-900 font-bold">Gespeicherte Zahlungsbedingungen</DialogTitle>
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-white">
        <DialogHeader>
          <DialogTitle className="text-gray-900 font-bold">Zahlungsbedingungen speichern</DialogTitle>
        </DialogHeader>
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <Label className="text-gray-900 font-medium">Name der Vorlage *</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} className="bg-white border-gray-300 text-gray-900" />
          </div>
          <div className="space-y-2">
