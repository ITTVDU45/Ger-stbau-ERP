...
// GET - Alle Einleitungstext-Vorlagen abrufen
export async function GET(request: NextRequest) {
  try {
    const db = await getDatabase()
    const vorlagenCollection = db.collection<EinleitungstextVorlage>('einleitungstext_vorlagen')
...
export async function POST(request: NextRequest) {
  try {
    ...
    const neueVorlage: EinleitungstextVorlage = {
      name: body.name,
      text: body.text,
