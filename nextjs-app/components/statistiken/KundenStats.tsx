...
/**
 * KundenStats - Statistik-Komponente für Kunden
 */
...
export function KundenStats({
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['statistiken-kunden', von.toISOString(), bis.toISOString()],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/statistiken/kunden?${params}`)
      if (!response.ok) {
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Angebotsquote</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {tables[0].daten.map((row: any, index: number) => (
                  <TableRow key={row.kundeId || index}>
                    <TableCell className="font-medium">{row.name}</TableCell>
                    <TableCell className="text-right">
                      {row.umsatz.toLocaleString('de-DE', { style: 'currency', currency: 'EUR' })}
                    </TableCell>
                    <TableCell className="text-right">{row.projekte}</TableCell>
                    <TableCell className="text-right">{row.angebotsquote.toFixed(1)}%</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
