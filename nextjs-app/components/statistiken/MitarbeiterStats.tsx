...
/**
 * MitarbeiterStats - Statistik-Komponente für Mitarbeiter
 */
...
interface MitarbeiterStatsProps {
  zeitraumTyp?: ZeitraumTyp
  initialVon?: Date
  initialBis?: Date
}

export function MitarbeiterStats({
  zeitraumTyp = 'monat',
  initialVon,
  initialBis
}: MitarbeiterStatsProps) {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['statistiken-mitarbeiter', von.toISOString(), bis.toISOString()],
    queryFn: async () => {
      ...
      const response = await fetch(`/api/statistiken/mitarbeiter?${params}`)
      if (!response.ok) {
              ...
              <TableBody>
                {tables[0].daten.map((row: any, index: number) => (
                  <TableRow key={row.mitarbeiterId || index}>
                    ...
                    <TableCell className="text-right">{row.einsaetze}</TableCell>
                    <TableCell className="text-right">{row.auslastung.toFixed(1)}%</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
